"""Binance USDT-M perpetual futures broker adapter.
...
class BinanceBroker(BaseBroker):
    """Binance USDT-M perpetual futures via ccxt."""
    ...
    async def place_order(
        ...
        try:
            ...
            logger.error("Binance place_order failed: %s", exc)
            return BrokerOrder(
    ...
    async def get_order(self, order_id: str, symbol: str = "") -> BrokerOrder | None:
        try:
            ...
            logger.error("Binance get_order failed: %s", exc)
            return None
    ...
    async def get_open_orders(self, symbol: str = "") -> list[BrokerOrder]:
        try:
            ...
            logger.error("Binance get_open_orders failed: %s", exc)
            return []
    ...
    async def get_position(self, symbol: str) -> BrokerPosition | None:
        try:
            ...
            logger.error("Binance get_position failed: %s", exc)
            return None
    ...
    def get_fee_rate(self, symbol: str = "") -> dict[str, float]:
        # Binance USDT-M default: 0.02% maker / 0.04% taker
    ...
    async def test_connection(self) -> bool:
        try:
            ...
            logger.warning("Binance test_connection failed: %s", exc)
            return False
