...
def run_agent_sync(prompt: str, max_turns: int = 3) -> str:
    """Run a single-turn agent query synchronously. Returns the response text.

    Lightweight wrapper around AgentLoop.run() for programmatic use (e.g.
    Indicator Lab AI code generation). Builds a minimal agent with all
    available tools and returns just the final text response.
    """
    ...
    try:
        ...
        _log.exception("run_agent_sync failed, returning empty string")
        return ""
