...
def _load_universe_panel(
    ...
    Args:
        ...
        use_cache: When True (default) reuse a pickle in
            ``~/.AStockPursue/cache/`` if the same universe+period was fetched
    ...
    cache_dir = Path.home() / ".AStockPursue" / "cache"
...
def _load_csi300_panel(start: str, end: str) -> dict[str, pd.DataFrame]:
    ...
    if not token or token == "your-tushare-token":
        raise RuntimeError(
            "TUSHARE_TOKEN not in backend/.env or environment; required for csi300 universe"
        )
...
def _fetch_sp500_constituents() -> list[str]:
    ...
    try:
        ...
        resp = requests.get(
            ...
            headers={
                "User-Agent": (
                    "AStockPursue/0.1 (research bench; "
...
def _default_output_dir() -> Path:
    return Path.home() / ".AStockPursue" / "reports"
...
class AlphaBenchTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "output_dir": {
                "type": "string",
                "description": "Where to write the HTML report; default ~/.AStockPursue/reports/.",
            },
        },
