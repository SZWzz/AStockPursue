#!/usr/bin/env python3
"""AStockPursue CLI for natural-language finance research and backtesting.

Usage:
    AStockPursue                           Interactive mode (default)
    AStockPursue -p "Backtest AAPL MACD"   Single run
    AStockPursue serve --port 8899         Start API server
    AStockPursue chat                      Interactive mode
    AStockPursue list                      List runs
    AStockPursue show <run_id>             Show run details
"""
...
class _RunDashboard:
    ...
    def render(self) -> Panel:
        ...
        return Panel(body, title="AStockPursue", border_style="cyan", padding=(1, 1 if compact else 2))
...
def _print_result(result: dict, elapsed: float, *, no_rich: bool = False) -> None:
    ...
    if result.get("run_id"):
        ...
        actions.add_row(f"AStockPursue show {rid}", "details")
        actions.add_row(f"AStockPursue code {rid}", "generated Python")
        actions.add_row(f"AStockPursue continue {rid} \"...\"", "refine this run")
...
def _build_welcome_panel(term_width: Optional[int] = None) -> Panel:
    ...
    title = f"AStockPursue v{_VERSION}"
    ...
    return Panel(body, title="[bold cyan]AStockPursue[/bold cyan]", border_style="cyan", padding=(1, 1))
...
def _show_settings() -> None:
    ...
    if compact:
        ...
        console.print(Columns(panels, expand=True, equal=True))
    console.print("[dim]Edit configuration in ~/.AStockPursue/.env, or run AStockPursue init.[/dim]")
...
def _cmd_db_setup() -> int:
    """Interactive setup: encrypt and store PostgreSQL credentials."""
    from pathlib import Path
    from src.config.paths import get_config_path

    console.print("[bold]PostgreSQL Database Setup[/bold]\n")
    host = Prompt.ask("Host", default="10.0.0.110")
    port = Prompt.ask("Port", default="5433")
    dbname = Prompt.ask("Database", default="stock-data")
    user = Prompt.ask("User", default="shenzhiwei")
    password = Prompt.ask("Password", password=True)

    from src.db.crypto import generate_key_b64, encrypt_password, decrypt_password
    ...
    try:
        ...
        if decrypted != password:
            console.print("[red]Encryption verification failed[/red]")
            return 1
    except Exception as e:
        console.print(f"[red]Encryption error: {e}[/red]")
        return 1

    env_path = get_config_path().parent / ".env"
    ...
    lines = [l for l in content.split("\n") if not l.startswith(("DB_HOST=", "DB_PORT=", "DB_NAME=", "DB_USER=", "DB_PASSWORD=", "DB_PASSWORD_ENC=", "DB_ENCRYPTION_KEY=", "SESSION_STORE="))]
    lines.append("")
    ...
    lines.append(f"DB_PASSWORD_ENC={enc}")
    lines.append(f"DB_ENCRYPTION_KEY={key}")
    lines.append("SESSION_STORE=pg")

    env_path.write_text("\n".join(lines), encoding="utf-8")
    console.print(f"\n[green]Database credentials encrypted and saved to {env_path}[/green]")
    return 0


def _cmd_db_rotate_key() -> int:
    """Rotate the database encryption key by re-encrypting the password."""
    import os
    from pathlib import Path
    from src.config.paths import get_config_path
    from src.db.crypto import generate_key_b64, encrypt_password, decrypt_password
    ...
    for line in content.split("\n"):
        ...
        if line.startswith("DB_PASSWORD_ENC="):
            old_enc = line.split("=", 1)[1].strip()

    if not old_key or not old_enc:
        console.print("[red]No encrypted password found. Run db-setup first.[/red]")
        return 1

    try:
        password = decrypt_password(old_enc, old_key)
    except Exception as e:
        console.print(f"[red]Failed to decrypt with old key: {e}[/red]")
        return 1

    new_key = generate_key_b64()
    new_enc = encrypt_password(password, new_key)
    ...
    for i, line in enumerate(lines):
        ...
        if line.startswith("DB_PASSWORD_ENC="):
            lines[i] = f"DB_PASSWORD_ENC={new_enc}"
    env_path.write_text("\n".join(lines), encoding="utf-8")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="AStockPursue CLI")
    parser.add_argument("--version", action="version", version=f"AStockPursue {_VERSION}")
    ...
    db_setup_parser = subparsers.add_parser("db-setup", help="Configure encrypted PostgreSQL connection")
    db_rotate_parser = subparsers.add_parser("db-rotate-key", help="Rotate database encryption key")

    subparsers.add_parser("init", help="Interactive setup: create ~/.AStockPursue/.env")
...
_INIT_ENV_PATH = Path.home() / ".AStockPursue" / ".env"
...
def cmd_memory_show(name: str, *, memory_dir: Optional[Path] = None) -> int:
    ...
    if entry is None:
        ...
        console.print("[dim]Run `AStockPursue memory list` to see available titles.[/dim]")
        return EXIT_USAGE_ERROR
...
def cmd_init() -> int:
    """Interactive setup: create ~/.AStockPursue/.env."""
    console.print(Panel("[bold cyan]AStockPursue setup[/bold cyan]\n[dim]Configure the default LLM provider and data tokens.[/dim]", border_style="cyan"))
    ...
    if key_env is not None:
        ...
        console.print("[dim]After setup, run: AStockPursue provider login openai-codex[/dim]")
    else:
    ...
    next_steps.add_row("Run", "[bold]AStockPursue[/bold]")
    if provider == "openai-codex":
        next_steps.add_row("OAuth", "[bold]AStockPursue provider login openai-codex[/bold]")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "provider":
        ...
        console.print("[red]provider requires a subcommand.[/red] Try: AStockPursue provider login openai-codex")
        return EXIT_USAGE_ERROR
    if args.command == "run":
    ...
    if args.command == "memory":
        ...
        console.print("[red]memory requires a subcommand.[/red] Try: AStockPursue memory list")
        return EXIT_USAGE_ERROR

    if args.command == "db-setup":
        return _coerce_exit_code(_cmd_db_setup())
    if args.command == "db-rotate-key":
        return _coerce_exit_code(_cmd_db_rotate_key())
