...
class TestSafeUserPath:
    def test_configured_import_root_file_accepted(self, tmp_path: Path, monkeypatch):
        monkeypatch.setenv("ASTOCKPURSUE_ALLOWED_FILE_ROOTS", str(tmp_path))
    ...
    def test_tilde_expansion_works(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("ASTOCKPURSUE_ALLOWED_FILE_ROOTS", "~/.AStockPursue-imports")
        target = tmp_path / ".AStockPursue-imports" / "journal.csv"
        target.parent.mkdir(parents=True, exist_ok=True)
        target.touch()

        result = safe_user_path("~/.AStockPursue-imports/journal.csv")
        assert result == target.resolve()
    ...
    def test_parent_traversal_from_cwd_rejected(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("ASTOCKPURSUE_ALLOWED_FILE_ROOTS", str(deep))
...
class TestSafeRunDir:
    def test_configured_run_root_accepted(self, tmp_path: Path, monkeypatch):
        monkeypatch.setenv("ASTOCKPURSUE_ALLOWED_RUN_ROOTS", str(tmp_path))
    ...
    def test_system_tmp_run_dir_rejected_by_default(self, tmp_path: Path, monkeypatch):
        monkeypatch.delenv("ASTOCKPURSUE_ALLOWED_RUN_ROOTS", raising=False)
