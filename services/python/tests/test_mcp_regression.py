...
2. MCP server mode regression — importing mcp_server.py must not raise, and
   ...
   confirms that the server-side MCP plugin (AStockPursue-mcp) is unaffected
...
# ---------------------------------------------------------------------------
# MCP server mode regression (AStockPursue-mcp plugin)
# ---------------------------------------------------------------------------


def _import_mcp_server():
    """Import backend/mcp_server.py without executing main().
...
def test_mcp_server_imports_without_raising() -> None:
    """backend/mcp_server.py must import cleanly (no side-effects on import)."""
    try:
