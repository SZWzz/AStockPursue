...
export function CorrelationMatrix({ symbols, matrix }: Props) {
  ...
  useEffect(() => {
    ...
    const cs = d3.scaleLinear<string>().domain([-1, 0, 1]).range([downColor, gridColor, upColor])
    ...
    matrix.forEach((row, i) => row.forEach((v, j) => {
      ...
      g.append('text').attr('x', j*cellSize+cellSize/2).attr('y', i*cellSize+cellSize/2).attr('text-anchor','middle').attr('dy','0.35em')
        .text(v.toFixed(2)).style('font-size',`${Math.max(9,cellSize/5)}px`).style('fill', Math.abs(v)>0.5?'#fff':'#5E6673').style('font-family','var(--font-mono)')
    }))
  }, [symbols, matrix])
  return (
