...
export function TradingWatchlist({ selectedSymbol, onSelect }: Props) {
  ...
  const handleStockSelect = async (code: string) => {
    ...
    setAdding(true);
    try {
      const res = await fetch("/v1/api/watchlist", {
        method: "POST",
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      {/* Stock search box — always visible */}
      <div className="flex items-center gap-1 px-3 py-2 border-b shrink-0">
        <div className="flex-1 min-w-0">
          <StockInput
            value={stockInput}
            onChange={(v) => { setStockInput(v); if (v.includes(".") || v.length >= 5) handleStockSelect(v); }}
            placeholder={t.tradingSearchStock || "搜索股票代码或名称…"}
          />
        </div>
        <button
          type="button"
          ...
          symbols.map((item) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-2 px-3 py-2 hover:bg-muted/50 transition text-left group border-b border-border/30",
