...
class RegimeEngine:
    ...
    def _classify(self, f: dict[str, float], market: str) -> tuple[str, float]:
        """Rule-based classification of current regime.

        Returns (regime, confidence).
        """
        # A-share specific checks
        if market == "CN_A":
            # These require additional data; fall back to universal rules
            pass

        # Universal rules (ordered by priority)
        if f["ema_gap_pct"] > 0.01 and f["directional_eff"] >= 0.55 and f["price_change_pct"] > 0.01:
            conf = min(0.95, 0.60 + abs(f["ema_gap_pct"]) * 10 + f["directional_eff"] * 0.3)
        ...
        if f["realized_vol_pct"] >= 0.045 or f["atr_pct"] >= 0.035:
            conf = min(0.95, 0.55 + f["realized_vol_pct"] * 5)
            return "high_volatility", conf

        if (abs(f["ema_gap_pct"]) <= 0.0045 and
                f["directional_eff"] <= 0.38 and
                f["atr_pct"] <= 0.02):
            conf = min(0.90, 0.50 + (0.02 - f["atr_pct"]) * 15)
