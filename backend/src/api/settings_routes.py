...
class UpdateLLMSettingsRequest(BaseModel):
    """Update LLM settings persisted to backend/.env."""
...
def _ensure_agent_env_file() -> Path:
    """Ensure the project-local backend/.env exists."""
...
def _read_settings_env_values() -> Dict[str, str]:
    """Read settings without creating backend/.env.

    Prefer the user's active backend/.env. If it does not exist yet, fall back to
    backend/.env.example for display defaults only.
    """
    if ENV_PATH.exists():
...
def _build_data_source_settings_response(values: Optional[Dict[str, str]] = None, *, token: Optional[str] = None,
    ...
    try:
        ...
        def check_one(name, cls, timeout=2):
            avail = False
            ...
            nonlocal yf_ok, tencent_ok, ccxt_ok, cg_ok, futu_ok, gi_ok, comm_ok
            nonlocal mootdx_ok, eastmoney_ok, baidu_ok
...
async def get_data_source_settings(auth: dict = Depends(_require_auth)):
    ...
    import asyncio
    user_id = auth["user_id"]
    ds_config = await asyncio.to_thread(_read_user_ds_config, user_id) if user_id > 0 else {}
...
async def get_skills(auth: dict = Depends(_require_auth)):
    """List all skills with enabled/disabled status."""
    from src.agent.skills import SkillsLoader
    user_id = auth["user_id"]
    config = _read_skill_config(user_id)
    disabled = set(config.get("disabled_skills", []))

    loader = SkillsLoader(user_id=user_id)
    skills = []
    ...
    return {"skills": skills, "total": len(skills), "enabled_count": sum(1 for s in skills if s["enabled"])}


@router.put("/settings/skills")
async def update_skills(payload: dict, auth: dict = Depends(_require_auth)):
    """Update disabled skills list."""
    user_id = auth["user_id"]
    disabled = payload.get("disabled_skills", [])
    _write_skill_config(user_id, {"disabled_skills": disabled})
    return {"ok": True}


@router.post("/settings/skills/import")
async def import_skill(req: Request, auth: dict = Depends(_require_auth)):
    """Import a custom skill from a ZIP upload."""
    user_id = auth["user_id"]
    form = await req.form()
    file = form.get("file")
    if not file:
    ...
    try:
        ...
        from pathlib import Path
        from src.agent.skills import _user_skills_dir
        dest = _user_skills_dir(user_id)
...
async def delete_skill(name: str, auth: dict = Depends(_require_auth)):
    """Delete a user-imported skill."""
    import shutil
    from src.agent.skills import _user_skills_dir
    user_id = auth["user_id"]
    skill_dir = _user_skills_dir(user_id) / name
    if skill_dir.exists():
