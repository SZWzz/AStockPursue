...
TEMPLATES: List[Dict[str, Any]] = [
    ...
    # ── 2. Dual MA crossover ───────────────────────────────────────────────
    {
        ...
        "nodes": [
            ...
            {"id": "t_entry", "node_type": "entry_signal", "label": "Entry",
             "position": {"x": 780, "y": -180}},
            {"id": "t_exit", "node_type": "exit_signal", "label": "Exit",
             "position": {"x": 780, "y": -20}},
            {"id": "t_hold", "node_type": "hold_signal", "label": "Hold",
             "position": {"x": 1040, "y": -100}, "config": {"initial": "flat"}},
        ],
        "edges": [
            ...
            {"id": "e7", "source": "t_golden", "source_port": "signal", "target": "t_entry", "target_port": "signal"},
            {"id": "e8", "source": "t_entry", "source_port": "entry", "target": "t_hold", "target_port": "enter"},
            {"id": "e9", "source": "t_death", "source_port": "signal", "target": "t_exit", "target_port": "signal"},
            {"id": "e10", "source": "t_exit", "source_port": "exit", "target": "t_hold", "target_port": "exit"},
        ],
    },
    ...
    },
    {
        ...
        "nodes": [
            ...
            {"id": "d_entry", "node_type": "entry_signal", "label": "Entry", "position": {"x": 780, "y": -180}},
            {"id": "d_exit", "node_type": "exit_signal", "label": "Exit", "position": {"x": 780, "y": -20}},
            {"id": "d_hold", "node_type": "hold_signal", "label": "Hold", "position": {"x": 1040, "y": -100}, "config": {"initial": "flat"}},
        ],
        "edges": [
            ...
            {"id": "de7", "source": "d_golden", "source_port": "signal", "target": "d_entry", "target_port": "signal"},
            {"id": "de8", "source": "d_entry", "source_port": "entry", "target": "d_hold", "target_port": "enter"},
            {"id": "de9", "source": "d_death", "source_port": "signal", "target": "d_exit", "target_port": "signal"},
            {"id": "de10", "source": "d_exit", "source_port": "exit", "target": "d_hold", "target_port": "exit"},
        ],
    },
    # ── Full research pipeline ──────────────────────────────────────────────
    {
        ...
        "description": "Factor → Screener → Backtest(fast+sim) → ConsistencyCheck → CostModel → Attribution",
        "patterns": {},
        "nodes": [
            {"id": "n_data", "node_type": "column_extract", "label": "close",
             "position": {"x": 0, "y": 0}, "config": {"column": "close"}},
            {"id": "n_factor", "node_type": "pct_change", "label": "Δ%(20)",
             "position": {"x": 260, "y": 0}, "config": {"periods": 20}},
            {"id": "n_screen", "node_type": "screener", "label": "Top 10",
             "position": {"x": 520, "y": 0}, "config": {"mode": "rank", "top_n": 10}},
            {"id": "n_fast", "node_type": "backtest", "label": "Backtest Fast",
             "position": {"x": 780, "y": -80}, "config": {"market": "equity_cn"}},
            {"id": "n_sim", "node_type": "backtest", "label": "Backtest Sim",
             "position": {"x": 780, "y": 80}, "config": {"market": "equity_cn", "_simulation_mode": True}},
            {"id": "n_check", "node_type": "consistency_check", "label": "Consistency",
             "position": {"x": 1040, "y": 0}},
            {"id": "n_cost", "node_type": "cost_model", "label": "Cost Model",
             "position": {"x": 1300, "y": -80}},
            {"id": "n_attr", "node_type": "attribution", "label": "Attribution",
             "position": {"x": 1300, "y": 80}},
        ],
        "edges": [
            {"id": "e1", "source": "n_data", "source_port": "series", "target": "n_factor", "target_port": "series"},
            {"id": "e2", "source": "n_factor", "source_port": "returns", "target": "n_screen", "target_port": "factor_data"},
            {"id": "e3", "source": "n_screen", "source_port": "filtered_codes", "target": "n_fast", "target_port": "codes"},
            {"id": "e4", "source": "n_screen", "source_port": "filtered_codes", "target": "n_sim", "target_port": "codes"},
            {"id": "e5", "source": "n_fast", "source_port": "backtest_result", "target": "n_check", "target_port": "fast_result"},
            {"id": "e6", "source": "n_sim", "source_port": "backtest_result", "target": "n_check", "target_port": "sim_result"},
            {"id": "e7", "source": "n_fast", "source_port": "backtest_result", "target": "n_cost", "target_port": "backtest_result"},
            {"id": "e8", "source": "n_fast", "source_port": "backtest_result", "target": "n_attr", "target_port": "backtest_result"},
        ],
    },
    # ── Walk-Forward validation ────────────────────────────────────────────
    {
        ...
        "description": "Regime detection → WalkForwardNode (multi-window param optimization) → OOS evaluation",
        "patterns": {},
        "nodes": [
            {"id": "n_data", "node_type": "column_extract", "label": "close",
             "position": {"x": 0, "y": 0}, "config": {"column": "close"}},
            {"id": "n_regime", "node_type": "regime", "label": "Regime",
             "position": {"x": 260, "y": 0}},
            {"id": "n_wf", "node_type": "walk_forward", "label": "Walk-Forward",
             "position": {"x": 520, "y": 0}, "config": {"n_windows": 5, "train_ratio": 0.7}},
            {"id": "n_compare", "node_type": "comparison", "label": "IS vs OOS",
             "position": {"x": 780, "y": 0}},
        ],
        "edges": [
            {"id": "e1", "source": "n_data", "source_port": "series", "target": "n_regime", "target_port": "ohlcv"},
            {"id": "e2", "source": "n_data", "source_port": "series", "target": "n_wf", "target_port": "ohlcv_data"},
        ],
    },
    # ── Turnover-aware strategy ────────────────────────────────────────────
    {
        "id": "turnover_aware_strategy",
        "name": "📉 Turnover-Aware Strategy",
        "description": "Factor → Rank → Weight → TurnoverConstraint → Backtest → CostModel",
        "patterns": {},
        "nodes": [
            {"id": "n_data", "node_type": "column_extract", "label": "close",
             "position": {"x": 0, "y": 0}, "config": {"column": "close"}},
            {"id": "n_factor", "node_type": "pct_change", "label": "Δ%(20)",
             "position": {"x": 260, "y": 0}, "config": {"periods": 20}},
            {"id": "n_select", "node_type": "rank_select", "label": "Top 10",
             "position": {"x": 520, "y": 0}, "config": {"top_n": 10}},
            {"id": "n_weight", "node_type": "signal_weight", "label": "Equal Weight",
             "position": {"x": 780, "y": 0}, "config": {"mode": "equal"}},
            {"id": "n_turnover", "node_type": "turnover_constraint", "label": "Turnover≤50%",
             "position": {"x": 1040, "y": 0}, "config": {"max_turnover": 0.5, "mode": "cap"}},
            {"id": "n_bt", "node_type": "backtest", "label": "Backtest",
             "position": {"x": 1300, "y": 0}, "config": {"market": "equity_cn"}},
            {"id": "n_cost", "node_type": "cost_model", "label": "Cost Model",
             "position": {"x": 1560, "y": 0}},
        ],
        "edges": [
            {"id": "e1", "source": "n_data", "source_port": "series", "target": "n_factor", "target_port": "series"},
            {"id": "e2", "source": "n_factor", "source_port": "returns", "target": "n_select", "target_port": "factor"},
            {"id": "e3", "source": "n_select", "source_port": "signal", "target": "n_weight", "target_port": "signal"},
            {"id": "e4", "source": "n_weight", "source_port": "signal", "target": "n_turnover", "target_port": "target_weights"},
            {"id": "e5", "source": "n_turnover", "source_port": "constrained_weights", "target": "n_bt", "target_port": "signal"},
            {"id": "e6", "source": "n_bt", "source_port": "backtest_result", "target": "n_cost", "target_port": "backtest_result"},
        ],
    },
    {
        "id": "demo_volbreak",
