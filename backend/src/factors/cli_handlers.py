"""CLI handlers for ``AStockPursue alpha {list,show,bench,compare,export-manifest}``.

All logic lives here; ``backend/cli.py`` only wires this in via :func:`add_subparser`
...
# ``<repo>/backend/src/factors/cli_handlers.py``; parents[3] is the repo root.
...
def _handle_exception(args: argparse.Namespace, prefix: str, exc: BaseException) -> int:
    ...
    if "TUSHARE_TOKEN" in str(exc):
        ...
        _err("  2. Add 'TUSHARE_TOKEN=<your_token>' to backend/.env  (or ~/.AStockPursue/.env)")
...
def cmd_alpha_list(args: argparse.Namespace) -> int:
    """``AStockPursue alpha list [--zoo X] [--theme Y] [--universe Z]``.
    ...
    try:
        ...
        if ids:
            ...
            _hint(
                f"Next: AStockPursue alpha show {example_id}  |  "
                f"Bench a zoo: AStockPursue alpha bench --zoo {example_zoo} "
...
def cmd_alpha_show(args: argparse.Namespace) -> int:
    """``AStockPursue alpha show <alpha_id> [--brief]`` — metadata + source."""
    try:
        reg = Registry()
        ...
        if brief:
            _hint(
                f"Next: AStockPursue alpha bench --zoo {alpha.zoo} "
...
def cmd_alpha_bench(args: argparse.Namespace) -> int:
    """``AStockPursue alpha bench --zoo X --universe csi300 --period Y-Z [--top N]``.
...
def cmd_alpha_compare(args: argparse.Namespace) -> int:
    """``AStockPursue alpha compare <id1> [<id2> ...] | --all | --zoo X``.
...
def cmd_alpha_export_manifest(args: argparse.Namespace) -> int:
    """``AStockPursue alpha export-manifest --out PATH [--force]``."""
    try:
...
def dispatch(args: argparse.Namespace) -> int:
    ...
    if sub is None:
        if _ALPHA_PARSER is not None:
            ...
            _err("alpha requires a subcommand. Try: AStockPursue alpha list")
        return 1
