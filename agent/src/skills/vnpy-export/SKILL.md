---
name: vnpy-export
description: Export a AStockPursue backtest strategy to a runnable vnpy CtaTemplate Python class — supports A-share equities, futures, and crypto via BarGenerator + ArrayManager.
category: tool
---

## Overview

This skill translates a AStockPursue strategy into a **vnpy `CtaTemplate` subclass** `.py` file
...
run a AStockPursue strategy inside vnpy's CTA backtester or live trading engine.
...
class {{StrategyName}}Strategy(CtaTemplate):
    """
    AStockPursue export — {{StrategyName}}
...
| Python (AStockPursue / pandas / ta-lib) | vnpy ArrayManager |
|----------------------------------------|-------------------|
...
| AStockPursue signal | Position check | vnpy call |
|---------------------|---------------|-----------|
...
When the AStockPursue strategy uses multiple timeframes (e.g., daily signal, hourly entry):
...
- [ ] Comment block at top of file notes the original AStockPursue `run_id` and instrument
