...
### Added
- [Git] Add `.workbuddy/` and `.opencode/` to `.gitignore`; untrack `.codegraph/daemon.pid` and `.workbuddy/` runtime files

### Fixed
- [CI] Fix golangci-lint `install-mode: goinstall` causing binary not found error — removed `install-mode` to use default download mode; fix `.golangci.yml` Go version from `1.25` to `1.22`
- [gRPC] Replace deprecated `grpc.DialContext`/`grpc.WithBlock` with `grpc.NewClient` + connectivity check in `connmgr.go`
